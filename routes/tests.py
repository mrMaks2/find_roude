...
class AllTestsCase(TestCase):
    ...
    def test_model_train_duplicate(self):
        ...
        train = Train(name='t1', from_city=self.city_A, to_city=self.city_B, travel_time=123)
        with self.assertRaises(ValidationError):
            train.full_clean()
    ...
    def test_valide_route_form(self):
        data = {
    ...
    def test_invalide_route_form(self):
        data = {
            'from_city': self.city_A.id,
            'to_city': self.city_B.id,
            'cities': [self.city_E.id, self.city_D.id]
